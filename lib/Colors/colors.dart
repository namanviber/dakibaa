import 'package:flutter/material.dart';

class AppTheme {

  /*final Color light_black = const Color(0xFFA5A8BC);
  final Color color_dark_gray = Color(0xFF42436A);
  final Color color_black = Colors.black;
  final Color color_white = const Color(0xFFFFFFFF);
  final Color color_gray = const Color(0xFFD9D9D9);
  final Color color_gray_light = const Color(0xFFA9A9A9);
  final Color color_blue = const Color(0xff3097F3);
  final Color color_royal_blue= Color(0xff191970);
  final Color color_logo= Color(0xff55585F);
  final Color color_gold=Color(0xffd7a81e);*/
  final Color wine_color=Color(0xff8e1520);
  /*final Color color_black = Colors.black;*/
  final Color color_black=Color(0xff313131);
  final Color color_white = const Color(0xFFFFFFFF);
  final Color color_red=Color(0xffD62525);

}